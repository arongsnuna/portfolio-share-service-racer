...
class Education {
    ...
    static async create({ userId, eduSchool, eduMajor, eduEnterDate, eduGraduateDate, eduDegree }) {
        const createdEducation = await EducationModel.create({eduSchool, eduMajor, eduEnterDate, eduGraduateDate, eduDegree, userId: userId});
        return createdEducation;
    }

    // 특정 유저의 학력 수정
    static async update({ userId, educationId, eduSchool, eduMajor, eduEnterDate, eduGraduateDate, eduDegree }) {
        const updatedEducation = await EducationModel.updateOne({userId: userId, _id: educationId},{eduSchool, eduMajor, eduEnterDate, eduGraduateDate, eduDegree});
